...
# TODODONE: In this cell, write your BaselineChallenge flow in the baseline_challenge.py file.
...
labeling_function = lambda row: 1 if row['rating'] >= 4 else 0  # TODODONE: Define your labeling function here.
...
class BaselineChallenge(FlowSpec):
    ...
    data = IncludeFile('data', default='../../full-stack-ml-metaflow-corise-week-1/data/Womens Clothing E-Commerce Reviews.csv')
    ...
    def baseline(self):
        ...
        major_preds = [major_class] * len(self.valdf)

        predictions = [major_class] * len(self.valdf) # TODODONE: predict the majority class
        acc = accuracy_score(self.valdf['label'], major_preds) # TODODONE: return the accuracy_score of these predictions
        rocauc = roc_auc_score(self.valdf['label'], major_preds) # TODODONE: return the roc_auc_score of these predictions
        self.result = ModelResult("Baseline", params, pathspec, acc, rocauc)
        self.next(self.aggregate)

    @step
    def model(self):
        # TODODONE: import your model if it is defined in another file.
        ...
        for params in self.hyperparam_set:
            model = NbowModel(vocab_sz = params['vocab_sz'])  # TODODONE: instantiate your custom model here!
            model.fit(X=self.traindf['review'], y=self.traindf['label'])

            acc = model.eval_acc(self.valdf['review'].values, self.valdf['label'])  # TODODONE: evaluate your custom model in an equivalent way to accuracy_score.
            rocauc = model.eval_rocauc(self.valdf['review'].values, self.valdf['label'])  # TODODONE: evaluate your custom model in an equivalent way to roc_auc_score.
    ...
    # @card # TODODONE: Set your card type to "corise". 
    ...
    def aggregate(self, inputs):
        ...
        # TODODONE: Add a Table of the results to your card! 
        current.card.append(
            Table(
                rows, # TODODONE: What goes here to populate the Table in the card? 
        ...
        # TODODONE: Append the matplotlib fig to the card
